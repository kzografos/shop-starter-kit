...
Layer map (blueprint §2, encoded in `verify-boundaries.js` and the audit script): INFRASTRUCTURE = `infrastructure/` (since E2, 2026-09-20: `prisma redis storage payments-provider mail health common` under it; before E2 they were seven top-level folders); CORE = `core/` (since E3a, 2026-09-22: `auth users profile staff settings notifications newsletter uploads` + `config events` under it); SHOP = `modules/` (`modules/ecommerce/{products categories favourites orders payments loyalty analytics}`); ROOT = `app.module.ts`, `main.ts`. Layer = top-level folder; architectural unit (cycles, rule E) = layer + first child. 116 files: CORE 52, SHOP 42, INFRASTRUCTURE 20, ROOT 2, unmapped 0.
...
| `core/profile/` — `ProfileService`, `ProfileController` (2) | CORE | core | `users`, `auth` (`PermissionsRegistryService` for `permissions[]`) | Ready | — | No Prisma access; applies user extensions |
| `core/staff/` — `StaffService`, `StaffController` (5) | CORE | core | `prisma` (`user`), `auth/permissions` (roles via re-export), `PermissionsRegistryService` | Ready | — | Last-owner guard |
| `core/settings/` — `SettingsService` (registry: `define/defineGroups`, `getAll/getPublic/updateAdminSettings`), `SettingDefinition`, controllers (3) | CORE / SHARED-CONTRACT (`setting-definition.ts`) | core | `prisma` (`setting`), `redis` (`settings:all`), `auth` | Ready | — | Shop contributes definitions (`orders/pricing-settings.ts`) — registry, not import into Core |
| `core/notifications/` — `NotificationsService` (rows, `createWrite`, `findOpen/create/update/resolveOpen`, unread caches), staff inbox controller (4), customer feed controller (4) | CORE | core | `prisma` (`notification`), `redis` (`notifications:unread:*`), `auth` | Ready | — | Row contract only; producers are Shop (`orders/order-notifications.service`, `products/stock-alerts.service`). `NotificationType` enum in `core.prisma` carries `LOW_STOCK`/`OUT_OF_STOCK`/`ORDER_STATUS` — see §7 (schema-level residue) |
| `core/newsletter/` — `NewsletterService`, public (2) + admin (1) controllers | CORE | core | `prisma` (`newsletterSubscriber`), `mail`, `ConfigService` (HMAC secret) | Ready | — | Welcome mail fire-and-forget (`MailService` logs) |
| `core/uploads/` — `UploadsService`, `UploadsController` (1) | CORE | core | `storage` (`StorageAdapter`), `auth` | Ready | — | Validation + `put()`; consumers: `products` (image sub-resource) |
...
|---|---|---|---|---|---|---|
| `modules/ecommerce/products/` — `ProductsService`, `StockAlertsService`, `CatalogPermissions`, public (4) + admin (8) controllers | SHOP | shop | `notifications` (`NotificationsService`), `uploads` (`UploadsService`), `storage`, `redis` (`products:*`), `auth` guards | Ready | — | Registers `view:catalog`/`manage:catalog`/`manage:inventory` + `stock_manager` preset |
| `modules/ecommerce/categories/` — service, public (1) + admin (4) | SHOP | shop | `products` (`invalidate()`), `redis` (`categories:tree`), `auth` guards | Ready | — | |
| `modules/ecommerce/favourites/` — service, controller (3) | SHOP | shop | `prisma` (`favourite`), `storage`, `auth` | Ready | — | |
| `modules/ecommerce/orders/` — `OrdersService`, `PricingSettingsService` (+ `pricing-settings.ts` definitions, public `GET /settings`), `GuestOrderLinkerService`, `OrderNotificationsService`, `OrdersPermissions` (+ `accountant` preset), `OrdersUserExtension`, `order-confirmation.mail.ts`, `order-status.ts`, controllers (4 + 2) | SHOP | shop | `products`, `settings` (registry), `users` (`UserExtensionsRegistry`), `loyalty`, `analytics`, `notifications`, `mail`, `storage`, `core/events` (subscribes), `common/after-commit`, `prisma.user` read (**F2**) | Ready after cleanup | F2 (optional) | `orders.service.create` does `prisma.user.findUniqueOrThrow` — Shop reading a Core model; a `UsersService.exists()` would remove it. Registers shop role presets (F10) because there is no shop root module yet |
| `modules/ecommerce/payments/` — `PaymentsService`, `PaymentsController` (3 incl. webhook) | SHOP | shop | `orders`, `loyalty`, `analytics`, `payments-provider` (`PaymentProvider`), `prisma.processedEvent`, `mail`, `common/after-commit` | Ready | — | Cross-sub-domain `order` writes inside the ledger transaction (F3, accepted) |
| `modules/ecommerce/loyalty/` — `LoyaltyService`, `LoyaltyUserExtension`, `LoyaltyController` (1) | SHOP | shop | `users` (`UserExtensionsRegistry`), `prisma` (`loyaltyAccount`, `loyaltyTransaction`) | Ready | — | Balance is `LoyaltyAccount.points`; earn/redeem/reverse only here |
| `modules/ecommerce/analytics/` — `AnalyticsService`, `AnalyticsPermissions`, controllers (1 + 1) | SHOP | shop | `redis` (`analytics:*`), `prisma.user` count with `MEMBER_ROLE` (**F2**) | Ready after cleanup | F2 (optional) | Registers `view:finance` |
...
| `project/utils/business.ts`, `project/composables/{useBusinessSchema,useOpeningHours}.ts` | PROJECT | Business facts (name, address, hours) — the blueprint's `project.config.ts`; in the project layer since E6a |
| `project/components/{ProjectBrandLockup,ProjectBrandWordmark,ProjectWhatsAppButton}.vue` | PROJECT / PRESENTATION | Brand; reached by Core only through the `brand` contribution and `globalWidgets` (E6a) |
...
|---|---|---|---|
| `backend/src/core/auth/guards/*`, `core/auth/decorators/*` | SHARED-CONTRACT (Core-owned) | core | Imported by every controller in all layers. Stays in Core; documented exception to the `users→auth` rule |
...
| ~~**P1**~~ | ~~Core shells/pages import project brand~~ **Closed 2026-09-22 (E6a).** `app/project/` is a Nuxt layer (`#project` alias, flat `Project*` component naming). The brand is `app.config.brand = { component: 'ProjectBrandLockup' }` rendered with `<component :is>` at nine Core sites; `ProjectWhatsAppButton` is a `globalWidget` (order 20, beside `ShopCartDrawer` order 10); `app.vue`, `AppFooter` and `unsubscribe` read name/legal name/tagline/favicon/ogImage/city/country from the `project` app.config block instead of importing `BUSINESS`. **Remaining:** `app.vue` → `useBusinessSchema()` (1 edge, composition root) | frontend | ownership / config | 1 CORE→PROJECT edge | — | — |
| ~~**F5**~~ | ~~`AppFooter` hard-codes `/products`, `/account/orders`; `ShopHeaderSearch` reads `header.search_placeholder`~~ **Closed 2026-09-22 (E6b).** The footer renders `app.config` `footerColumns` + `footerItems` through `app/utils/footer-registry.ts`: Core contributes the `account` column and `/account`, the shop layer the `shop` column with `/products` and the two account entries, the project layer the `info` column with `/about`, `/contact` and the (link-less, icon-bearing) shipping note. Which i18n keys carry the project's footer copy is named by `app.config.project.footer`. `ShopHeaderSearch` uses its own `products.search_placeholder` key in the shop layer's locale files | frontend | presentation | — | — | — |
...
| ~~**F10**~~ | ~~Shop role presets registered from `orders/`~~ **Closed 2026-09-22 (E3b).** `modules/ecommerce/ecommerce.module.ts` imports the seven sub-modules and provides `EcommercePermissions` (`ecommerce-permissions.ts`, owner of `SHOP_ROLE_PRESETS`); `OrdersPermissions` keeps `ORDERS_CAPABILITIES` only; `AppModule` imports `EcommerceModule`. Behaviour pinned (`test/permissions-registry.test.mjs`) and compared pre/post on a scratch DB: registry outputs, `/profile` permissions and the 5×12 admin matrix identical. Note: sub-modules still import each other directly (orders/payments → analytics, loyalty), so removing a sub-module from the root only removes it when nothing else imports it — the Module Registry (E8) is where enable/disable becomes real | backend | composition | — | — | — |
...
**Baseline at this commit** (from the script, after F1, E2, E3a and E3b): backend 118 files / 412 edges (408 + the shop root's 4 net edges) / forbidden **0** / cycles 0 / unit cycle 1 (`core/auth↔core/users`; units are layer + first child since E3a); frontend 91 files / 193 edges / forbidden 12 (all CORE→PROJECT) / Core→Shop via registry 5 / cycles 0 / unresolved 8.
...
5. ~~Shop role presets (F10)~~ done in E3b (`EcommercePermissions` at the shop root). Residual: `core/staff/dto/staff.dto.ts` still validates `role` against a literal `['admin','accountant','stock_manager']` — Core naming Shop roles; replace with a registry-backed check (`staffRoles()`) in a later slice.
