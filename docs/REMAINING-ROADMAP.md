...
| ~~E6c~~ | ~~**The last Core→Project edge**~~ **done 2026-09-22** — `app/project/plugins/business-schema.ts` (universal layer plugin) calls `useBusinessSchema()`; `app.vue` keeps only Core SEO mechanics. Frontend **Core→Project = 0, forbidden = 0** | ~~E6b~~ | gates green, SSR one `ld+json` / one `Store` on `/`, `/en`, `/en/login`, `/en/products`, still 1 across client navigation, five UI suites pass |
| E7 | **Frontend Core layer** `app/core` + project layer (`app.config.ts`, brand, `business.ts`, its plugin, home/about/contact). The brand/footer/schema ownership questions are settled by E6a–E6c — E7 moves files, it does not revisit that edge | E5, E6 | `--strict` audit passes |
