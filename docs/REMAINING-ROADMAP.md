...
| E3b | **Shop root module** `modules/ecommerce/ecommerce.module.ts`; `accountant`/`stock_manager` presets move from `orders/orders-permissions.ts` to the ecommerce-level registrar (F10); `AppModule` imports `EcommerceModule` | E3a | providers scenarios, `permissionsFor()`/`/profile` permissions identical, routes 65/65 |
