...
|---|---|---|---|---|---|
| ecommerce (root) | `modules/ecommerce/ecommerce.module.ts`, `ecommerce-permissions.ts` | Shop composition root: imports the seven sub-domain modules; `EcommercePermissions` registers the shop staff-role presets (`accountant`, `stock_manager`) — E3b/F10, 2026-09-22 | `EcommerceModule` (exports nothing) | the seven sub-domains, core/auth (registry) | `app.module.ts` |
...
| storefront shell | `layouts/default.vue`, `components/layout/{AppHeader,AppFooter}.vue` (the brand mark and the WhatsApp button are project contributions since E6a) | Header/footer/widgets from `app.config` contribution lists | layout `default` | app.config, auth store, permissions, useApi (newsletter) | every storefront page |
...
| F10 | ~~**Shop role presets registered from `orders/`**~~ **Resolved (E3b, 2026-09-22):** `modules/ecommerce/ecommerce.module.ts` is the shop root (imports the seven sub-modules, exports nothing); `ecommerce-permissions.ts` owns `SHOP_ROLE_PRESETS` and `EcommercePermissions` registers them; `OrdersPermissions` registers order capabilities only. Outputs pinned by `backend/test/permissions-registry.test.mjs`; pre/post `permissionsFor`, `/profile` and the admin 401/403/200 matrix identical | `modules/ecommerce/ecommerce.module.ts`, `ecommerce-permissions.ts`, `orders/orders-permissions.ts`, `app.module.ts` | — | closed |
...
> | Frontend Core layer | **P1 closed by E6a** — the brand is an `app.config.brand` contribution, the WhatsApp button a `globalWidget`, business data a `project` app.config block; 12 Core→Project edges → 1 (`app.vue` calls the project `useBusinessSchema()`). F5 footer links remain (E6b) | F5 |
...
- Project layer (`app/project/` since E6a: brand components, `utils/business.ts`, the two business composables, its `app.config.ts`): plus `brand.css`, `pages/{index,about,contact}.vue`, `nuxt.config.ts` locales, `prisma/seed.ts` owner bootstrap — Phase 3 (`project.config.ts`) work, not layer work.
