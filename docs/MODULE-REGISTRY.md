...
| storefront shell | `layouts/default.vue`, `components/layout/{AppHeader,AppFooter}.vue`, `utils/footer-registry.ts` (the brand mark and the WhatsApp button are project contributions since E6a; the footer columns/items are contributions since E6b) | Header/footer/widgets from `app.config` contribution lists | layout `default` | app.config, auth store, permissions, useApi (newsletter) | every storefront page |
