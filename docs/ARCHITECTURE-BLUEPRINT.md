...
| **Project frontend** | `app/project/` layer (brand components, `utils/business.ts`, business composables, its `app.config.ts`; E6a), `pages/{index,about,contact}`, `brand.css` values, fonts, `project.config.ts`, project i18n namespaces (`home`, `hero`, `about`, `contact`, `testimonials`), any client-specific layouts or workflows |
