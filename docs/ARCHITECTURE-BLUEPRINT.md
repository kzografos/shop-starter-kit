...
- Make optional providers optional — **DONE (backend)**: `core/config/env.validation.ts` holds the core schema plus conditional provider fragments; the storage adapter's MinIO client, the payment provider's Stripe client, Resend transport and Google strategy are instantiated only when configured and fail with 503 when used unconfigured; `rawBody` documented in `main.ts`. Still open: Nginx `/media/` block is unconditional (Docker config, separate step); frontend still renders the Google button regardless.
