...
| **E-commerce module** (`ecommerce.prisma`) | `Product`, `Category`, `Favourite`, `Order`, `OrderItem`, `LoyaltyTransaction`, enums `OrderStatus`, `FulfillmentType`, `PaymentMethod`, `PaymentStatus`, `LoyaltyType` | Adds `LoyaltyAccount { userId @unique, points }`. Its relation fields on `User` (`orders`, `favourites`, `loyaltyTransactions`, later `loyaltyAccount`) are written inside the `User` block in `core.prisma` (one model = one block) and marked there as shop-owned |
...
| 3b | **`UserRole` enum → `role String`** — **step 1 DONE (schema split, no DB change)**; **step 2 open (role-type migration)** | `UserRole` enum still has `CUSTOMER`, `ACCOUNTANT`, `STOCK_MANAGER`; `staff.service` demotes to `UserRole.CUSTOMER`; `'CUSTOMER'` literal in `users.service`, `analytics.service` | `role String`; Core names only `ADMIN` (owner) and the member role; new staff roles become registrations, not migrations. Values stay `customer/admin/accountant/stock_manager` in this step (no API change); any rename is a later, explicit contract change | Postgres enum → text migration (`USING role::text`); requires the multi-file Prisma schema first (D11) and a rehearsed migration for the downstream (PetShopCY) — highest-risk migration in v1 |
...
- Remove shop-specific roles from Core (seam 3) — **3a DONE (code)**: capability/role vocabulary is a module-fed registry (`auth/permissions.registry.service.ts`). **3b open**: `UserRole` enum → `role String` schema migration, after the multi-file schema split. Frontend `STAFF_ROLES` duplication stays for Phase 2; `orders-permissions.ts` may move with the folder/layer structure.
