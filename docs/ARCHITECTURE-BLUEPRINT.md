...
- Loyalty extraction (seam 2) — **DONE** ahead of the layer split: `backend/src/loyalty/` owns `LoyaltyAccount` + ledger, Core exposes `UserExtensionsRegistry` so module fields reach client payloads without Core naming the module; contracts preserved (D13). Still open here: storage adapter (seam 7), payments provider surface (seam 6), cache namespace hygiene (seam 8).
