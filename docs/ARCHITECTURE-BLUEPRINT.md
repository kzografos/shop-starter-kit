...
| Generic events | In-process event bus (to be added — does not exist today) |
...
|---|---|---|---|---|
| 1 | **Remove guest-order linking from Core Auth** | `AuthService.linkGuestOrders()` runs a transaction over `order`, `loyaltyTransaction`, `user.loyaltyPoints`, `setting` on register/login/Google login (`backend/src/auth/auth.service.ts:44-78`) | E-commerce `orders` subscribes to `user.authenticated` / `user.registered` events and links orders itself | Requires the event bus (D8) first. Behavioural: linking becomes asynchronous relative to token issue — the first `/profile` after login may not yet show linked orders. Acceptable; document it |
...
| 4 | **Move shop settings definitions into E-commerce** | `SettingsService.PRICING_SETTING_KEYS`, `loadPricing()`, public `GET /settings`, `admin.service.SETTING_KEYS`, `seedSettings()`, admin settings form fields | Settings Registry entries in the e-commerce module; Core `SettingsService` becomes generic (`define`, `get<T>`, `getPublic`, `set`, `invalidate`) | Low. `/settings` response shape stays the same for the shop; the checkout page keeps working |
| 5 | **Separate generic email/notification mechanisms from shop templates** | `MailService.sendOrderConfirmation`; `NotificationsService.checkStock`, `LOW_STOCK_THRESHOLD`, `NotificationType` enum, `productId`/`stock` columns, `manage:inventory` guard on the inbox | Mail: transport + layout in Infrastructure, `send({to, subject, html})`; order template in e-commerce. Notifications: Core `notify({type, key?, meta})` + `resolve(key)`; stock rule in e-commerce `catalog`; inbox guarded by `view:notifications` | Low. Notification column migration is a small backfill |
...
| 10 | **Split the God Admin service** | `backend/src/admin/admin.service.ts` (515 lines): stats, orders, products, categories, customers, newsletter, settings | Customers → Core `users` (`GET /admin/users`); newsletter → Core `newsletter`; settings → Core `settings`; the rest → e-commerce sub-domain admin controllers (pattern already used by `staff`, `notifications`, `analytics`) | Low. Route paths can be preserved during the move to avoid frontend churn |
