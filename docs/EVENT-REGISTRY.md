...
| Sync/async | **Inside the status transaction** (`createWrite` = `ON CONFLICT DO NOTHING`); unread-cache invalidation after commit | Awaited on catalogue writes; detached after order commit via `afterCommit()` (`infrastructure/common/utils/after-commit.ts`) |
