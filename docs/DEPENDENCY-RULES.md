...
|---|---|---|
| Boundaries | `backend/scripts/verify-boundaries.js` | §2–§3 (Core/Infra must not import shop folders; **Infra must not import Core folders** — rule A, widened when F1 closed), §4 (no service imports a controller), §6.1 (no shop capability on a Core controller), §6.4 (no shop Prisma model in a Core service), rule E (`users/` imports `auth/` only for guards/decorators — `auth` depends on `users`, never the reverse) — on the **current layout** (`infrastructure/` is a real folder since E2; Core and Shop folders are still flat under `src/`), via the folder→layer map from the blueprint §2 |
