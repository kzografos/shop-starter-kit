...
3. `ProcessedEvent` and similar technical ledgers are Infrastructure; modules use them through the pattern (insert inside the same transaction), not by adding domain columns to them. Its reference column is the neutral `subjectId` (text) — the former `orderId` was renamed by `20260919120000_processed_event_subject`; do not reintroduce a domain-named column.
...
|---|---|---|
| Boundaries | `backend/scripts/verify-boundaries.js` | §2–§3 (Core/Infra must not import shop folders; **Infra must not import Core folders** — rule A, widened when F1 closed), §4 (no service imports a controller), §6.1 (no shop capability on a Core controller), §6.4 (no shop Prisma model in a Core service), rule E (`core/users` imports `core/auth` only for guards/decorators — `auth` depends on `users`, never the reverse) — on the **layered layout** (`src/infrastructure/` since E2, `src/core/` and `src/modules/ecommerce/` since E3a): layer = top-level folder, architectural unit = layer + first child (`core/auth`, `modules/ecommerce/orders`, `infrastructure/mail`) |
...
| Unit tests | `npm test` (node `--test`, runs after build inside `verify`) | `afterCommit()` helper; the extraction audit script's positive/negative tests — the positive test pins the boundary baseline (no Core→Shop edge backend or frontend, only the 3 F1 INFRA→CORE edges, `auth↔users` the only folder cycle), so a regression fails CI |
| Extraction audit (manual) | `npm run audit:extraction` (`-- --json`, `-- --strict`) — `backend/scripts/audit-extraction-readiness.js` | File-level import/DI/Prisma/auto-import graph for **both** trees, cross-layer edges, cycles, unresolved relationships; not a verify step on its own (frontend baseline has 12 documented CORE→PROJECT edges), see EXTRACTION-READINESS §14 |
...
| ~~frontend `usePermissions.STAFF_ROLES` / `SECTION_CAPS` duplicate the role/section vocabulary~~ | §5.2 | **Removed (Admin Registry).** Sections come from `app.config.adminSections`; `isStaff` derives from the resolved permissions |
...
| ~~`backend/prisma/seed.ts` `seedSettings()` hard-codes the five pricing keys; `app/pages/admin/settings/index.vue` hard-codes the same five form fields~~ | §8.3 | **Removed (Settings Registry).** Defaults live in `orders/pricing-settings.ts` and are applied by `SettingsService.getAll()`; the form renders `GET /admin/settings` definitions (`docs/SETTINGS-REGISTRY.md`) |
...
| ~~`backend/src/users/users.service.ts` imports `MEMBER_ROLE` from `auth/permissions` (users → auth at runtime)~~ | Core internal direction | **Removed (pre-extraction cleanup, seam 1).** Role values live in `users/roles.ts`; `auth/permissions.ts` re-exports them; boundary rule E enforces the direction. Guard/decorator imports from `auth/` are the shared contract and are allowed everywhere |
| ~~frontend `stores/auth.ts` exposes `loyaltyPoints`; `types/index.ts` `Profile.loyalty_points`~~ | §5.2 (Core store/type naming a module field) | **Removed (seam 2).** Shop `useLoyalty()` reads the user-extension field; `Profile` carries extensions as `[extension: string]: unknown`; `LoyaltyProfileExtension` is Shop-owned |
| ~~frontend `useCustomerNotifications.describe()` phrases `order_status` rows~~ | §5.2 | **Removed (seam 3).** Core presenter registry (`app/utils/notification-presenters.ts`); Shop registers `order_status` from `plugins/order-notifications.ts` |
| ~~`ProcessedEvent.orderId`~~ | §7.3 | **Removed (seam 4).** `subjectId` text column, migration `20260919120000_processed_event_subject` |
| ~~`storage/storage.module.ts`, `payments-provider/stripe.provider.ts`, `mail/mail.service.ts` import `core/config/env.validation.ts` (Infrastructure → Core)~~ | §2 (Infrastructure depends on nothing above it) | **Removed (F1, EXTRACTION-READINESS §8).** Each provider defines its own presence rule (`isStorageConfigured`, `isPaymentsConfigured`, `isMailConfigured`) next to the code it switches; the boot schema stays in `core/config` and is imported only by `app.module.ts` and `auth/`. Rule A now fails any INFRA → CORE import |
...
| `backend/src/infrastructure/common/interceptors/snake-case.interceptor.ts` `ENUM_FIELDS` names order enums | §3 | E7 |
...
| ~~`app/modules/ecommerce/components/filters/ShopHeaderSearch.global.vue` (shop) reads the Core i18n key `header.search_placeholder`~~ | §5.8 | **Removed (E6b).** The component uses `products.search_placeholder` from the shop layer's own locale files; the Core key is left in place for Core's own use |
...
| `app/project/composables/useBusinessSchema.ts` hardcodes `Store`, `priceRange`, `currenciesAccepted`; `useCurrency` hardcodes EUR / `el-GR` | §8.2 | Phase 3 |
