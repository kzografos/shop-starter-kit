...
| `UserRole` Prisma enum still carries `CUSTOMER`, `ACCOUNTANT`, `STOCK_MANAGER`; `staff.service` demotes to `UserRole.CUSTOMER`; `'CUSTOMER'` literal in `users.service`, `analytics.service`; frontend `usePermissions.STAFF_ROLES` / `SECTION_CAPS` duplicate the vocabulary | §3, §7.1, §5.2 | seam 3b (schema; after the multi-file schema split) · Phase 2 Admin Registry (frontend) |
